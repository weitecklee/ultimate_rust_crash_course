...
fn main() {
    ...
    if let Some(fruit) = maybe_fruit {
        println!("It's a fruit! {fruit}")
    }
    ...
    let snacks = [&healthy_snack, &sugary_snack, &lunch];
    for snack in snacks {
        match *snack {
    ...
    let snacks = [Snack::Apple, Snack::Cookies(5), Snack::Apple];
...
fn do_math(x: i32) -> Result<i32, String> {
    ...
    Err(format!("I wanted the number 1 and you gave me a {} 🥺", x))
}

fn inspect(x: Option<&str>) {
    if let Some(s) = x {
        println!("You passed in a {s}")
    }
}
